...
impl Server {
    ...
    fn handle_connection(
        ...
        match commands[0] {
            ...
            "LEAVE" => {
                let db = db.lock().unwrap();
                let mut stmt = db.prepare("SELECT has_left FROM servers WHERE ip_address = ?1")?;
                let result =
                    stmt.query_row([address.ip().to_string()], |row| row.get::<_, bool>(0));
                match result {
                    Ok(has_left) => {
                        if has_left {
                            ...
                            db.execute(
                                "UPDATE servers SET has_left = true WHERE ip_address = ?1",
